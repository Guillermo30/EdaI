...
public class AVLTree<T>	extends AbstractSet<T> {
   ...
   private int modCount;
    ...
    private AVLNode<T> copy(AVLNode p){
    	return null;
    }

    @SuppressWarnings("unchecked")
	public boolean equals(Object obj){
		return equals(this.root,((AVLTree<T>)obj).root);
    }
    
    private boolean equals(AVLNode<T> p, AVLNode<T> q){
    	if ((p==null) || (q==null))
			return q==p;
		if ((!p.nodeValue.equals(q.nodeValue)) || (p.height!=q.height))
			return false;
		if (equals(q.left,p.left)&&equals(q.right,p.right))
			return true;
		return false;
    }
    ...
    */
	...
	public void removeAll(ArrayList<Integer> aList) {
		java.util.Iterator<Integer> iter = aList.iterator();
    	while(iter.hasNext())    	
    	     remove(iter.next());
	}

	public boolean containsAll(ArrayList<Integer> aList) {
		java.util.Iterator<Integer> iter = aList.iterator();
    	while(iter.hasNext())
	...
	public boolean containsAll(AVLTree<Integer> otherAVLTree) {
		Iterator iter = otherAVLTree.iterator();
    	while(iter.hasNext())
    	     if(!contains(iter.next()))
    	       return false;
    	return true;
	...
	public void retainAll(AVLTree<Integer> avlTree) {
		AVLTree<T> auxTree=new AVLTree<T>();
	...
	public void addAll(ArrayList<Integer> aList) {
		java.util.Iterator<Integer> iter = aList.iterator();
    	while(iter.hasNext())    	
    	     add((T) iter.next());
	}

	public void removeAll(AVLTree<Integer> otherAVLTree) {
		Iterator iter = otherAVLTree.iterator();
    	while(iter.hasNext())    	
    	     remove(iter.next());
	}
	...
	private static class AVLNode<T>
	{
		...
		public AVLNode(T item, int height, AVLNode<T> left, AVLNode<T> right)
		{
			this.nodeValue = item;
			this.left = left;
			this.right = right;
