      ...
      implements List<T>, Iterable<T>, Cloneable, java.io.Serializable
{
   ...
   public Object[] toArray()
   {
		...
		throws java.io.IOException
	{
		...
		// AQUI
		try{
			out.writeInt(this.listArr.length);
			for(int i=0; i<this.listArr.length; i++){
				out.writeObject(this.listArr[i]);
			}
		}catch(Exception e){
			System.out.println("los objetos del array no son serializables");
		}
		...
		throws IOException, ClassNotFoundException
	{
		...
		// AQUI
		try{
			T[] aux = (T[]) new Object[in.readInt()];
			for(int i=0; i<aux.length; i++){
				aux[i] = (T) in.readObject();
			}
			this.listArr = (T[]) aux;
		}catch(Exception e){
