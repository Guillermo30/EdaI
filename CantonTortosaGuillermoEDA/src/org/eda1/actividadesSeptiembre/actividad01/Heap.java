...
public class Heap<T> {
	 ...
	 * 
	 * @param start
	 *            - el indice del Heap donde va a empezar la restauracicn de la
	 *            propiedad.
	 */
	...
	public boolean buscaTopDown() {
		...
		for (int child = (parent << 1) + 1; child < theHeap.size(); child = (parent << 1) + 1) {
			...
			if (child < theHeap.size() - 1
					&& compare(getValue(child), getValue(child + 1)) > 0)
	...
	public void delete(int index) {
		...
		Integer decremento = (Integer) theHeap.get(index) - (Integer) getMin()
				+ new Integer(1);
	...
	public Iterator<T> breadthFirstIterator() {
		...
		return new HeapBreadthFirstIterator();
	}

	private class HeapBreadthFirstIterator implements Iterator<T> {
		// Iterator<T> salida = this.breadthFirstIterator();
		...
		public T next() {
			...
			T salida;
			//int contador=-1;
			salida = theHeap.get(contador);
			contador++;
			
			return salida;
			/*
			if (contador == theHeap.size()){
				return null;
			}else if(contador == 0){
					salida = theHeap.get(contador+1);
					contador++;
				return salida;
				...
				if(izquierda == true){
					izquierda = false;
					salida = theHeap.get(2*contador);
					contador++;
					return salida;
					
				}else{
					izquierda= true;
					salida = theHeap.get(2*contador+1);
					contador++;
					return salida ;
				}
		...
		public void remove() {
			// TODO Auto-generated method stub
			int pos = theHeap.indexOf(contador);
			delete(pos);
		}
	...
	public Iterator<T> depthFirstIterator() {
		// TODO Auto-generated method stub
		return new HeapDepthFirstIterator();
	}

	private class HeapDepthFirstIterator implements Iterator<T> {
		// Iterator<T> salida = this.breadthFirstIterator();
		//copiamos el heap a una lista
		ArrayList<T> listaIteracion = oredenarEnProfundidad();
		Iterator<T> it = listaIteracion.iterator();
		...
		private ArrayList<T> oredenarEnProfundidad(){
			...
			if(!theHeap.isEmpty()){
				...
				while(!lista.isEmpty()){
					...
					if((2*padre+1)<=(theHeap.size()-1) && lista.contains(theHeap.get(2*padre+1))){
						...
						if(lista.contains(theHeap.get(padre))){
							
							cola.push(theHeap.get(padre));
							lista.remove(theHeap.get(padre));	
						}
						
						if(padre!=0){	
							
							if((2*padre+2)<=(theHeap.size()-1)&&lista.contains(theHeap.get(2*padre+2))){
								
								hijo = 2*padre+2;
								padre = hijo;
								
								while((2*padre+1)<=(theHeap.size()-1)&&lista.contains(theHeap.get(2*padre+1))){
									hijo=2*padre+1;		
									padre=hijo;
								}	
								if(lista.contains(theHeap.get(padre))){
