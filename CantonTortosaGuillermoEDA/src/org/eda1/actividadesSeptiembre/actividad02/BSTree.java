...
public class BSTree<T> implements Collection<T>, Iterable<T>, Cloneable,
	...
	public BSTree(BSTree<Integer>  otherTree) {
		root = copyTree((BSTNode<T>) otherTree.root);
		modCount = otherTree.modCount;
	...
	public boolean equals(Object obj) {
		...
		}else if(!root.nodeValue.equals(other.root.nodeValue)){
			return false;
		}
	...
	public void removeSmaller(int i) {
	...
	private BSTNode<T> removeLeaves(BSTNode<T> tree){
	...
	public int sumOfSmallerThanOrEqualTo(int i) {
		int suma=0;
		int other;
		Iterator it=this.iterator();
	...
	public int countOutOfRange(int i, int j) {
		int salida=0;
		int	other;
		Iterator it=this.iterator();
	...
	public int countNodesBetweenLevels(int i, int j) {
		int salida=0;
		T elemento;
		Iterator it=this.iterator();
		...
		while(it.hasNext()){
			...
			if((this.pathHeight(elemento)>=i)&&(this.pathHeight(elemento)<=j)){
				salida++;
			}
		}
		return salida;
	...
	public String toStringReverseBreadthFirstTraversal() {
		// TODO Auto-generated method stub
		ALStack<BSTNode<T>> pila=new ALStack<BSTNode<T>>();
		LinkedQueue<BSTNode<T>> cola=new LinkedQueue<BSTNode<T>>();
		BSTNode<T>node =null;
		String salida="";
